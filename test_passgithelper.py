...
class HelperConfig:
    ...
    The ``helper_config`` fixture also makes this data available to the test
    implementation by returning it as part of the ``HelperConfigAndMock`` object
    after test setup is done. To access it, tests must explicitly add
    ``helper_config`` to their function signature. Used in this way allows tests
    to adapt their behavior to the provided parametrization data. It also allows
    tests to configure the teardown part of the ``helper_function`` fixture (see
    ``mock_co_expect_call`` below).
    ...
    Attributes:
        ...
        request:
            Used by ``helper_config`` to simulate the request data provided by
            git via ``stdin`` (default: "").
        entry_data:
            ...
            ``subprocess.check_output`` mock which simulates calling
            ``pass``. If ``entry_data`` is provides, the setup done by
            ``helper_config`` mimics a successful ``pass``word retrieval.
            With ``entry_data=None`` (the default) or ``entry_data=b""``,
            ``helper_config`` mimics a missing password store entry.
        entry_name:
            Name of the password store entry (aka `pass_target`) to retrieve. It
            mainly gets used in the teardown part of ``helper_config`` to check
            that it is contained in the the command line options provided to
            ``pass`` (default: None). It can also be used to adapt
            test implementation behaviour to a given parametrization.
        patch_check_password_file:
            When set to False (which is the default), ``helper_config`` will
            patch the ``check_password_file(..)`` function. Only change this to
            ``True`` for tests of the ``check_password_file(..)`` function.
        mock_co_expect_call:
            With the default (``True``), ``helper_config`` will add
            ``assert_called_*`` checks for the ``subprocess.check_output`` mock
            during teardown. When set to ``False``, these asserts will be
            skipped.
        out_expected:
            Document the expected output on ``stdout`` in the test
            parametrization. Will automatically be checked when using the
            ``teardown_helper_capsys_checks(..)`` function.
        err_expected:
            Same as ``out_expected``, but for ``stderr`` instead of
            ``stdout``.
    ...
    patch_check_password_file: bool = True
...
class HelperConfigAndMock:
    ...
    Attributes:
        ...
        mock_co: Mock for subprocess.check_output as used in the
            passgithelber.get_password(..) function.
    """
...
@pytest.mark.parametrize(
    ...
    argvalues=[
        ...
        (["get"], {"PASS_GIT_HELPER_SKIP_FS_CHECKS": "11"}, False),
...
host=example.com""",
    ...
    def test_uses_password_store_dir_relative_to_home(
        ...
        env: dict[str, str] = mock_co.call_args.kwargs["env"]
...
class TestScriptCheckPasswordFile:
    ...
    def test_function_check_password_file_with_mocked_pass(
        ...
        """Test function ``check_password_file(..)`` with mocked ``pass`` executable.

        The test runs ``passgithelper.main(..)`` while mocking sub-process calls
        to the ``pass`` executable. The filesystem checks of
        ``check_password_file(..)`` operate on ``test_data/dummy-password-store``.
        ...
        setup_helper_parse_request_mock(
            ...
            use_wildcard_section=True,
            username_extractor="regex_search",
            # make sure that check_password_file uses
    ...
    def test_option_skip_fs_checks(
        self,
        mocker: MockerFixture,
        capsys: CapsysType,
        caplog: CaplogType,
        ...
        if not test_params.err_expected:
            ...
            caplog_record = (
                "root",
                logging.DEBUG,
                "File system level checks for password store files are disabled",
            )
