...
class DataInserter(abc.ABC):
    ...
    @abc.abstractmethod
    def set_value(
            self, entry_name: Text, entry_data: Text, value: Text
        ) -> Text: 
...
class TemplateInserter(DataInserter):
    def __init__(self, template: Text, field_name: Text = "username", option_suffix: Text = "") -> None:
        """Create a new instance.
...
_username_inserters = {
    ...
    #TODO: EntryNameInserter
    #"entry_name": EntryNameInserter(option_suffix="_username"),
}
...
def set_password(
    ...
    password_inserter = TemplateInserter("${password}", "password", option_suffix="_password")
    ...
    subprocess.run(["pass", "insert", "--multiline", "--force", pass_target], input=pass_content, encoding=section.get("encoding", "UTF-8"), text=True, env=environment)
def get_password(
    request: Mapping[str, str], mapping: configparser.ConfigParser
) -> None:
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    if action == "get":
        ...
        try:
            ...
            print(  # noqa: T201
                "Unable to store entry: {error}".format(error=error), file=sys.stderr
            )
            sys.exit(1)
    else:
